 ...
 */
const proxyIPs = ['ProxyIP.US.CMLiussss.net', 'oracle.sosoorg.com'];
			...
			if (request.headers.get('Upgrade') !== 'websocket') {
				...
				if (matchingUserID) {
					if (url.pathname === `/${matchingUserID}` || url.pathname === `/sub/${matchingUserID}`) {
						...
						const bestIps = await getBestips(host, proxyAddresses);
						const content = isSubscription ?
							await GenSubBest(bestIps, matchingUserID, host) :
...
// 获取优选ip代替优选域名
async function getBestips(hostname, proxyIP) {
	const mainDomains = new Set([
		hostname,
		// public domains
		'time.is', 'ip.sb', 'malaysia.com', 'russia.com', 'singapore.com', 'skk.moe',
		'www.visa.com', 'www.visa.com.hk', 'www.visa.com.tw', 'www.visa.co.jp', 'www.visakorea.com',
		'www.gco.gov.qa', 'www.gov.se', 'www.gov.ua', 'www.digitalocean.com', 'www.csgo.com', 'www.whoer.net',
		'www.whatismyip.com', 'www.ipget.net', 'www.hugedomains.com', 'www.udacity.com', 'www.4chan.org',
		'www.okcupid.com', 'www.glassdoor.com', 'www.udemy.com', 'www.baipiao.eu.org',
		// 高频率更新
		'freeyx.cloudflare88.eu.org', 'cloudflare.182682.xyz', 'cfip.cfcdn.vip', proxyIP,
		// 手动更新和未知频率
		'cf.0sm.com', 'cloudflare-ip.mofashi.ltd', 'cf.090227.xyz', 'cf.zhetengsha.eu.org', 'cloudflare.9jy.cc',
		'cf.zerone-cdn.pp.ua', 'cfip.1323123.xyz', 'cdn.tzpro.xyz', 'cnamefuckxxs.yuchen.icu', 'cfip.xxxxxxxx.tk',
	]);

	const bestipUrl = 'https://api.hostmonit.com/get_optimization_ip';
	const bestipBody = {
		key: 'iDetkOys'
	};

	try {
		// 发送 POST 请求，使用 await 等待请求完成
		const response = await fetch(bestipUrl, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
			body: JSON.stringify(bestipBody)
		});

		const data = await response.json(); // 等待解析 JSON 数据

		if (data.code === 200) {
			const updatedMainDomains = new Set();
			data.info.forEach(item => {
				updatedMainDomains.add(item.ip); // 将 IP 加入 Set，Set 自动去重
			});

			return updatedMainDomains;
		} else {
...
async function GenSubBest(bestIps, userID_path, hostname) {
	...
	for (const userID of userIDArray) {
		for (const ip of bestIps) {
			let urlPartGov = await GenUrlPart(ip);
			// console.log(`IP: ${ip}, Location: ${urlPartGov}`); // Debug 日志
			const urlPart = urlPartGov || `${hostname.split('.')[0]}-${ip}-HTTPS-${port}`;
			const url = atob(pt) + '://' + userID + atob(at) + ip + ':' + port + commonUrlPartHttps + urlPart;;
			allUrls.push(url);
		}
	}
